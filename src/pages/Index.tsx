...
import { createPredictionReportFromExistingPrediction, getAdminOrchestrationSnapshot } from '@/services/predictionReportService';
  ...
  const handleTimeLocked = useCallback(async (
    ...
    try {
      ...
      if (qResult.unifiedResult) {
        setUnifiedReport(createPredictionReportFromExistingPrediction(qResult.unifiedResult.input, qResult));
      }
      setStep('result');
                ...
                {isSuperAdmin && unifiedReport && (
                  <TabsContent value="orchestration" className="mt-6">
                    <div className="space-y-4">
                      ...
                      <AdminOrchestrationConsole profile={profile} snapshot={getAdminOrchestrationSnapshot(unifiedReport, profile)} />
                    </div>
                  </TabsContent>
                )}
              </Tabs>
