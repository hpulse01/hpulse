...
function buildAdminSnapshot(report: Omit<FullPredictionReport, 'adminSnapshot'>, result: QuantumPredictionResult): AdminOrchestrationSnapshot {
  ...
  const coverageMatrix = report.engineResults.map((engineResult) => ({
    ...
    pseudoImplementationRisk: engineResult.eventCandidates.length > 0 ? 'low' : engineResult.confidence >= 0.65 ? 'medium' : 'high',
...
function buildReport(input: UnifiedPredictionInput, prediction: QuantumPredictionResult): FullPredictionReport {
...
export const PredictionOrchestrator = {
  ...
  fromPrediction(input: UnifiedPredictionInput, prediction: QuantumPredictionResult): FullPredictionReport {
    return buildReport(input, prediction);
  },
