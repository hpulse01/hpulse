          ...
          <tbody>
            {list.map((e) => (
              <tr key={e.engineName} className="border-t border-primary/10 hover:bg-primary/[0.03]">
                ...
                <td className="px-3 py-2 text-right font-mono text-primary/90">{((e.confidence ?? 0) * 100).toFixed(0)}%</td>
                <td className="px-3 py-2 text-right font-mono">{(e.completenessScore ?? 0).toFixed(0)}</td>
        ...
        {list.map((e) => (
          <li key={e.engineName} className="rounded-lg border border-primary/15 bg-card/40 p-3">
            ...
            <div className="grid grid-cols-4 gap-2 mt-3 text-[10px] font-mono">
              <div><div className="text-muted-foreground/60">Conf</div><div className="text-primary/90">{((e.confidence ?? 0) * 100).toFixed(0)}%</div></div>
              <div><div className="text-muted-foreground/60">Compl</div><div>{(e.completenessScore ?? 0).toFixed(0)}</div></div>
